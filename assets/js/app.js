...
function updateLeaveDates() {
    const teamPrevDateElements = document.querySelectorAll(
        ".team_prev_details__dates",
    );
    ...
    const myPrevStartDateElements = document.querySelectorAll(
        "#start-date.my_prev__details__date",
    );
    const myPrevEndDateElements = document.querySelectorAll(
        "#end-date.my_prev__details__date",
    );
    if (myPrevStartDateElements && myPrevEndDateElements) {
        myPrevStartDateElements.forEach(function (startDateElement) {
            startDateElement.textContent = formatDate(
                startDateElement.dataset.start,
            );
        });
...
function checkForNotifications() {
    ...
    if (notificationPanel && notificationButton) {
        const hasNotifications =
            notificationPanel.querySelector(".notification") !== null;
...
function initializeTableFilters() {
    ...
    const filterTable = () => {
        ...
        const table =
            document.querySelector(".my_leaves_list__table") ||
            document.querySelector(".requests_lists__table");
        if (!table) return;
